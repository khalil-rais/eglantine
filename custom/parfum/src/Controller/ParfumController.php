...
class ParfumController extends ContainerBuilder {
	...
	//private $user = "test";private $password = "drop325";
	...
	function _generation_pdf($order) {
		...
		$pdf->Text(155, 28, utf8_decode(t('Djerba the').' '.date('d/m/Y',strtotime($order ['order_time'])).',' ));//Djerba le
		...
		$pdf->SetFont('arial', 'B', 12);
		//$pdf->SetXY(5, 39);
		$pdf->Text(7, 42, utf8_decode('Sté Eglantine Nature'));	
		$pdf->SetFont('arial', '', 12);
		$pdf->Text(7, 48, utf8_decode('1 rue youssef labbassi'));	
		$pdf->Text(7, 54, utf8_decode('El May 4175 Djerba'));	
		$pdf->SetFont('arial', 'B', 12);
		$pdf->Text(7, 60, utf8_decode(t('Phone').' :'));
		$pdf->Text(7, 66, utf8_decode(t('Tax identifier').' :'));
		
		$pdf->SetFont('arial', '', 12);
		$pdf->Text(32, 60, utf8_decode('92227397'));	
		$pdf->Text(9, 72, utf8_decode('000/M/A/1675662/R'));	
		...
		if (isset ($order['lines'])){//We will draw the taken paramters.
			...
			$w_totaux = array ('Sous Total H.T','Frais de Port H.T','Fiscal stamp', 'TOTAL H.T');
			$w_totaux_text = array (t('Sub Total'),t('Shipping fees'),t('Fiscal stamp'), 'TOTAL');
...
private function integration_article ($value,$volume_array,&$marques_list){
	...
	public function removeProductAttribute($offset=1) {
		...
		foreach ($list_of_values as $key => $value){
			$volume_array [$key]= $value->getName();
			$attribute_value = $value->getName();
			if ($counter < $offset){
				$value->delete();
			}
			$counter++;
		}		
		$count_volume_array = count($volume_array)-$offset;
		return array(
		  '#type' => 'markup',
		  '#markup' => 'Attributs Existants: '.$count_volume_array.'.',
		);
	}
